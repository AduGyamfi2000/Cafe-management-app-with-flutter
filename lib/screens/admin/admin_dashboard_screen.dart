...
class _AdminDashboardScreenState extends State<AdminDashboardScreen> {
  ...
  Future<void> _persistMenuItems() async {
    ...
    await prefs.setString(
      'menu_items',
      jsonEncode(_menuItems.map((item) => item.toJson()).toList()),
    );
    setState(() {
      _saving = false;
    });
  ...
  Future<void> _showEditDialog({MenuItem? menuItem}) async {
    ...
    setState(() {
      if (isNew) {
        final id = _menuItems.isEmpty
            ? 1
            : (_menuItems.map((e) => e.id).reduce((a, b) => a > b ? a : b) + 1);
                ...
                children: [
                  ...
                  Expanded(
                    child: _menuItems.isEmpty
                        ? const Center(
                            child: Text(
                              'No menu items. Add one using the button below.',
                            ),
                          )
                        : ListView.builder(
